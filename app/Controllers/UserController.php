...
class UserController extends BaseController
{
    ...
    public function __construct()
    {
	    ...
	    public function jumptoshop()
    {
        return $this->view()->display('user/shop.tpl');
    }
    ...
    public function node($request, $response, $args)
    {
        ...
        if ($user->is_admin) {
          ...
          $ports_count = Node::where(
				...
				// by Ls 8-17		低等级可见高等级线路
            )->where('type', 1)->where('sort', 9)->/*where("node_class", "<=", $this->user->class)->*/orderBy('name')->count();
